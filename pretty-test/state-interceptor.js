// ================= CONFIGURATION =================
// Set to "ALL" to watch all tables, or a specific visual name like "Prettyico116"
const WATCHED_TABLE = "ALL";
// =================================================

(async function () {
  console.clear();
  console.log("📡 State Interceptor & Logger Initializing...");
  ...
  function handleTableStatePacket(packet) {
    ...
    if (newStatus !== prevStatus) {
      ...
      console.log(
        `\n%c ⚡ TRANSITION %c ${displayName} %c State changed from [ ${labelPrev} ] to ➡️ [ ${labelNew} ]`,
        ...
        "background: #1e1e1e; color: #a9b7c6; padding: 2px 6px; border-radius: 0 4px 4px 0;"
      );
      ...
      if (pinia && pinia._s) {
        ...
        for (const key of storeKeys) {
          if (key.toLowerCase().includes('baccarat')) {
            ...
            if (store && store.setSocket && !store.setSocket.isStateInterceptorHooked) {
              const originalSetSocket = store.setSocket;
              store.setSocket = function (roomId, packet) {
                try {
                  if (packet && roomId) {
                    // Inject roomId if missing
                    if (!packet.roomId) packet.roomId = roomId;
                    handleTableStatePacket(packet);
                  }
                } catch (e) { }
                return originalSetSocket.apply(this, arguments);
              };
              store.setSocket.isStateInterceptorHooked = true;
              console.log(`🔌 Pinia: Intercepted Baccarat Store "${key}" state updates directly!`);
            }
          }
        }
      }
  ...
  function hookSocketIoInstance(socket) {
    if (!socket || socket._stateInterceptorHooked) return;
    socket._stateInterceptorHooked = true;
    ...
    socket.onevent = function (packet) {
      try {
        ...
        if (payload) {
          ...
          if (tableData) {
            handleTableStatePacket(tableData);
          }
        }
      } catch (err) {
        // Safe catch
  ...
  function hookWebSocketInstance(ws) {
    if (!ws || ws._wsStateInterceptorHooked) return;
    ws._wsStateInterceptorHooked = true;
  ...
  if (window.io && !window.io.isStateInterceptorHooked) {
    ...
    window.io.isStateInterceptorHooked = true;
    Object.assign(window.io, originalIo);
    console.log("🔌 window.io Hook Registered!");
  }

  console.log(`🔌 [INTERCEPTOR RUNNING] Currently watching: %c${WATCHED_TABLE}%c`, "color: #00ff00; font-weight: bold;", "");
})();
